...
public class MainBSTree{ 
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException{
    	...
    	BSTMovieHashTable hash=new BSTMovieHashTable(11);
       ...
       while( FileParsers.hasNextEvent() ){
           ...
           if(event.getOperation().equals("I")){ // operation
           	...
           	if(tree.movieExists(event.getMovieId())){  //h tainia yparxei
           		System.out.println("Success ["+tree.get(event.getMovieId()).getTitle()+"]"); 
      ...
      //menu
        ...
        while(aaaaa){
        	...
        	if(fun==1){
        		...
        		String temp=null;
        		double rank=( ((tree.getRoot().getRating()*tree.getRoot().getVotes())+(6.9*25000))/(tree.getRoot().getVotes() + 25000) );
                double max=rank;
                MovieBSTreeNode current=tree.getRoot();
                ...
                while(current!=null){
                	...
                	if(current.getlChild()!=null){
                		...
                		rank=( ((current.getRating()*current.getVotes())+(6.9*25000))/(current.getVotes() + 25000) );
                		if(rank>max){
                     		max=rank;
                ...
                while(node !=null){
                	dur+=tree.get(node.getMovieId()).getDuration();
                	node=node.getNext();	
                }
                ...
                while(user1 != null){
                	...
                	while(movie !=null){
                		if(tree.get(movie.getMovieId()).getGenres().contains(eidos)){
                			counter++;
                		}
                		movie=movie.getNext();
                	}
            	...
            	genre_t eidos=genre_t.valueOf(word);
                 ...
                 double min_rank=( ((ak.getRating()*ak.getVotes())+(6.9*25000))/(ak.getVotes() + 25000) );
                MovieBSTreeNode current=tree.getRoot();
                MovieBSTreeNode parent=null;
                while(current!=null){
                	if(parent!=null){
